  ...
  render(attrs: Record<string, string>, body: string, t: ThemeColors): string {
    ...
    if (level === 1) {
      ...
      const titleBlock = hasNum && hide !== 'num'
        ? `<strong style="display:block;font-size:${titleFontSize};font-weight:900;color:${titleColor};line-height:1.26;letter-spacing:-0.8px;margin-top:${titleMarginTop};margin-left:${titleMarginLeft}">${hasPrefix ? prefix + ' ' : ''}${leaf(title)}${hasSuffix ? ' ' + suffix : ''}</strong>`
        : `<strong style="display:block;font-size:${titleFontSize};font-weight:900;color:${titleColor};line-height:1.26;letter-spacing:-0.8px">${hasPrefix ? prefix + ' ' : ''}${leaf(title)}${hasSuffix ? ' ' + suffix : ''}</strong>`
      const subtitleHtml = subtitle
        ? `<span style="display:block;margin-left:${hasNum && hide !== 'num' ? subtitleMarginLeft : '0'};font-size:${subtitleFontSize};color:${subtitleColor};font-weight:700;text-transform:uppercase;letter-spacing:1.6px">${leaf(subtitle)}</span>`
        : ''
...
<section style="margin:36px 0px 20px;overflow:hidden">
    ...
    <section style="margin-left:32px;font-size:24px;font-weight:800;color:${titleColor};line-height:1.4;letter-spacing:-0.3px">${titleText}</section>
</section>`
      }
      return `
<section style="margin:36px 0px 20px">
  <section style="font-size:24px;font-weight:800;color:${titleColor};line-height:1.4;letter-spacing:-0.3px">${titleText}</section>
</section>`
    }
...
<section style="margin:28px 0px 16px;overflow:hidden">
    ...
    <section style="margin-left:32px;font-size:20px;font-weight:700;color:${titleColor};line-height:1.45">${titleText3}</section>
</section>`
      }
      return `
<section style="margin:28px 0px 16px">
  <section style="font-size:20px;font-weight:700;color:${titleColor};line-height:1.45">${titleText3}</section>
</section>`
    }
...
<section style="margin:24px 0px 12px;overflow:hidden">
    ...
    <section style="margin-left:32px;font-size:16px;font-weight:700;color:${titleColor};line-height:1.5">${titleText4}</section>
</section>`
    }
    return `
<section style="margin:24px 0px 12px">
  <section style="font-size:16px;font-weight:700;color:${titleColor};line-height:1.5">${titleText4}</section>
</section>`
  },
}
